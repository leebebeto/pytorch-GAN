...
class Generator(nn.Module):
	def __init__(self):
		...
		self.layer1 = nn.Linear(args.latent_vector + 10, 256)
...
class Discriminator(nn.Module):
	def __init__(self):
		...
		self.embedding = nn.Embedding(10,10)
		self.layer1 = nn.Linear(784 + 10, 512)
...
for epoch in range(args.epoch):
	for i, (images, labels) in enumerate(train_loader):
		...
		condition_vector = label2vec(labels)
		real_images = images.to(device)

		if real_images.shape[0] < args.batch_size: break 
		generated_image = generator(random_vector, condition_vector).view(args.batch_size,1,28,28)

		# train discriminator		
		d_loss = criterion(discriminator(real_images, condition_vector), torch.ones(args.batch_size,1)) + criterion(discriminator(generated_image, condition_vector), torch.zeros(args.batch_size,1))
		...
		# train generator
		g_loss = criterion(discriminator(generated_image, condition_vector), torch.ones(args.batch_size,1))
...
with torch.no_grad():
	...
	for i in range(16):
		i = i % 10
		condition_vector = torch.zeros(1,10)
		condition_vector[0][i] = 1
		test_image = torch.randn(1, args.latent_vector)
		test_image = generator(test_image, condition_vector).view(1,28,28)
